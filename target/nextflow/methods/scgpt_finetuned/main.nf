// scgpt_finetuned build_bugfix_no_ref_adjust_resources
// 
...
meta = [
  ...
  "namespace" : "methods",
  "version" : "build_bugfix_no_ref_adjust_resources",
  "argument_groups" : [
    {
      "name" : "Arguments",
      "arguments" : [
  ...
  "build_info" : {
    "config" : "/home/runner/work/task_label_projection/task_label_projection/src/methods/scgpt_finetuned/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
    "output" : "target/nextflow/methods/scgpt_finetuned",
    "viash_version" : "0.9.0",
    "git_commit" : "007cc441d171548c92f229959e1ddf8e4ce7d1c6",
...
print(f"input_train: {input_train}", flush=True)
print(f"input_test: {input_test}", flush=True)

################### PREPROCESS DATA ###################
# Make the batch_id column
train_batch_str = input_train.obs["batch"].astype(str)
test_batch_str = input_test.obs["batch"].astype(str)
unique_batches = np.unique(np.concatenate((train_batch_str, test_batch_str)))
num_batch_types = len(unique_batches)

input_train.obs["batch_id"] = np.searchsorted(unique_batches, train_batch_str)
input_test.obs["batch_id"] = np.searchsorted(unique_batches, test_batch_str)

# del train_batch_str, test_batch_str

# Make the celltype id column
input_train.obs["celltype"] = input_train.obs["label"].astype("category")
unique_celltypes = input_train.obs["celltype"].cat.categories
...
mask_value = -1
pad_value = -2
max_seq_len = input_train.n_vars + 1
...
explicit_zero_prob = (
    training_settings["MLM"] and hyperparameters["include_zero_gene"]
)  # whether explicit bernoulli for zeros
...
gene_ids_in_vocab = np.array(input_train.var["id_in_vocab"]) + np.array(
    input_test.var["id_in_vocab"]
)
...
preprocessor(input_train, batch_key="batch")
print(f"input_train: {input_train}", flush=True)
preprocessor(input_test, batch_key="batch")
print(f"input_test: {input_test}", flush=True)
...
(
    ...
    all_counts,
    np.array(input_train.obs["celltype_id"].tolist()),
    np.array(input_train.obs["batch_id"].tolist()),
    test_size=0.1,
    shuffle=True,
)
...
for name, para in model.named_parameters():
    if (
        hyperparameters["freeze"]
        and "encoder" in name
        and "transformer_encoder" not in name
    ):
...
scheduler = torch.optim.lr_scheduler.StepLR(
    optimizer, schedule_interval, gamma=hyperparameters["schedule_ratio"]
)
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openproblems-bio/task_label_projection/methods/scgpt_finetuned",
    "tag" : "build_bugfix_no_ref_adjust_resources"
  },
  "label" : [
    "midtime",
    "highmem",
